...
// once by TestMain — the same arrangement the repo package uses.
//
// setupPipeline used to start (and terminate) its own Postgres and Mongo per
// test, so a two-test package paid four container starts and gave the
// container-readiness race four chances to bite. Its per-test retry loop
// around Schema.Create documented that race but only papered over it — CI
// still hit "connection reset by peer" past the loop's budget. TestMain now
// waits both containers out with backoff once, before any test runs, and each
// test gets its own database (see newPostgresDSN and mongoDatabase) for the
// same clean slate a dedicated container gave it.
var (
