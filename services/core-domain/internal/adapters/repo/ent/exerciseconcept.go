// Code generated by ent, DO NOT EDIT.

package ent

import (
	"fmt"
	"strings"
	"time"

	"entgo.io/ent"
	"entgo.io/ent/dialect/sql"
	"github.com/google/uuid"
	"github.com/motifpath/core-domain/internal/adapters/repo/ent/concept"
	"github.com/motifpath/core-domain/internal/adapters/repo/ent/exercise"
	"github.com/motifpath/core-domain/internal/adapters/repo/ent/exerciseconcept"
)

// ExerciseConcept is the model entity for the ExerciseConcept schema.
type ExerciseConcept struct {
	config `json:"-"`
	// ID of the ent.
	ID int `json:"id,omitempty"`
	// ExerciseID holds the value of the "exercise_id" field.
	ExerciseID uuid.UUID `json:"exercise_id,omitempty"`
	// ConceptID holds the value of the "concept_id" field.
	ConceptID uuid.UUID `json:"concept_id,omitempty"`
	// LinkedAt holds the value of the "linked_at" field.
	LinkedAt time.Time `json:"linked_at,omitempty"`
	// Edges holds the relations/edges for other nodes in the graph.
	// The values are being populated by the ExerciseConceptQuery when eager-loading is set.
	Edges        ExerciseConceptEdges `json:"edges"`
	selectValues sql.SelectValues
}

// ExerciseConceptEdges holds the relations/edges for other nodes in the graph.
type ExerciseConceptEdges struct {
	// Exercise holds the value of the exercise edge.
	Exercise *Exercise `json:"exercise,omitempty"`
	// Concept holds the value of the concept edge.
	Concept *Concept `json:"concept,omitempty"`
	// loadedTypes holds the information for reporting if a
	// type was loaded (or requested) in eager-loading or not.
	loadedTypes [2]bool
}

// ExerciseOrErr returns the Exercise value or an error if the edge
// was not loaded in eager-loading, or loaded but was not found.
func (e ExerciseConceptEdges) ExerciseOrErr() (*Exercise, error) {
	if e.Exercise != nil {
		return e.Exercise, nil
	} else if e.loadedTypes[0] {
		return nil, &NotFoundError{label: exercise.Label}
	}
	return nil, &NotLoadedError{edge: "exercise"}
}

// ConceptOrErr returns the Concept value or an error if the edge
// was not loaded in eager-loading, or loaded but was not found.
func (e ExerciseConceptEdges) ConceptOrErr() (*Concept, error) {
	if e.Concept != nil {
		return e.Concept, nil
	} else if e.loadedTypes[1] {
		return nil, &NotFoundError{label: concept.Label}
	}
	return nil, &NotLoadedError{edge: "concept"}
}

// scanValues returns the types for scanning values from sql.Rows.
func (*ExerciseConcept) scanValues(columns []string) ([]any, error) {
	values := make([]any, len(columns))
	for i := range columns {
		switch columns[i] {
		case exerciseconcept.FieldID:
			values[i] = new(sql.NullInt64)
		case exerciseconcept.FieldLinkedAt:
			values[i] = new(sql.NullTime)
		case exerciseconcept.FieldExerciseID, exerciseconcept.FieldConceptID:
			values[i] = new(uuid.UUID)
		default:
			values[i] = new(sql.UnknownType)
		}
	}
	return values, nil
}

// assignValues assigns the values that were returned from sql.Rows (after scanning)
// to the ExerciseConcept fields.
func (_m *ExerciseConcept) assignValues(columns []string, values []any) error {
	if m, n := len(values), len(columns); m < n {
		return fmt.Errorf("mismatch number of scan values: %d != %d", m, n)
	}
	for i := range columns {
		switch columns[i] {
		case exerciseconcept.FieldID:
			value, ok := values[i].(*sql.NullInt64)
			if !ok {
				return fmt.Errorf("unexpected type %T for field id", value)
			}
			_m.ID = int(value.Int64)
		case exerciseconcept.FieldExerciseID:
			if value, ok := values[i].(*uuid.UUID); !ok {
				return fmt.Errorf("unexpected type %T for field exercise_id", values[i])
			} else if value != nil {
				_m.ExerciseID = *value
			}
		case exerciseconcept.FieldConceptID:
			if value, ok := values[i].(*uuid.UUID); !ok {
				return fmt.Errorf("unexpected type %T for field concept_id", values[i])
			} else if value != nil {
				_m.ConceptID = *value
			}
		case exerciseconcept.FieldLinkedAt:
			if value, ok := values[i].(*sql.NullTime); !ok {
				return fmt.Errorf("unexpected type %T for field linked_at", values[i])
			} else if value.Valid {
				_m.LinkedAt = value.Time
			}
		default:
			_m.selectValues.Set(columns[i], values[i])
		}
	}
	return nil
}

// Value returns the ent.Value that was dynamically selected and assigned to the ExerciseConcept.
// This includes values selected through modifiers, order, etc.
func (_m *ExerciseConcept) Value(name string) (ent.Value, error) {
	return _m.selectValues.Get(name)
}

// QueryExercise queries the "exercise" edge of the ExerciseConcept entity.
func (_m *ExerciseConcept) QueryExercise() *ExerciseQuery {
	return NewExerciseConceptClient(_m.config).QueryExercise(_m)
}

// QueryConcept queries the "concept" edge of the ExerciseConcept entity.
func (_m *ExerciseConcept) QueryConcept() *ConceptQuery {
	return NewExerciseConceptClient(_m.config).QueryConcept(_m)
}

// Update returns a builder for updating this ExerciseConcept.
// Note that you need to call ExerciseConcept.Unwrap() before calling this method if this ExerciseConcept
// was returned from a transaction, and the transaction was committed or rolled back.
func (_m *ExerciseConcept) Update() *ExerciseConceptUpdateOne {
	return NewExerciseConceptClient(_m.config).UpdateOne(_m)
}

// Unwrap unwraps the ExerciseConcept entity that was returned from a transaction after it was closed,
// so that all future queries will be executed through the driver which created the transaction.
func (_m *ExerciseConcept) Unwrap() *ExerciseConcept {
	_tx, ok := _m.config.driver.(*txDriver)
	if !ok {
		panic("ent: ExerciseConcept is not a transactional entity")
	}
	_m.config.driver = _tx.drv
	return _m
}

// String implements the fmt.Stringer.
func (_m *ExerciseConcept) String() string {
	var builder strings.Builder
	builder.WriteString("ExerciseConcept(")
	builder.WriteString(fmt.Sprintf("id=%v, ", _m.ID))
	builder.WriteString("exercise_id=")
	builder.WriteString(fmt.Sprintf("%v", _m.ExerciseID))
	builder.WriteString(", ")
	builder.WriteString("concept_id=")
	builder.WriteString(fmt.Sprintf("%v", _m.ConceptID))
	builder.WriteString(", ")
	builder.WriteString("linked_at=")
	builder.WriteString(_m.LinkedAt.Format(time.ANSIC))
	builder.WriteByte(')')
	return builder.String()
}

// ExerciseConcepts is a parsable slice of ExerciseConcept.
type ExerciseConcepts []*ExerciseConcept
