...
// QueryLocale chains the current query on the "locale" edge.
func (_q *UserQuery) QueryLocale() *LanguageQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withLocale: _q.withLocale.Clone(),
...
// WithLocale tells the query-builder to eager-load the nodes that are connected to
// the "locale" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithLocale(opts ...func(*LanguageQuery)) *UserQuery {
	query := (&LanguageClient{config: _q.config}).Query()
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withLocale; query != nil {
		if err := _q.loadLocale(ctx, query, nodes, nil,
			func(n *User, e *Language) { n.Edges.Locale = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadLocale(ctx context.Context, query *LanguageQuery, nodes []*User, init func(*User), assign func(*User, *Language)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "locale_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *UserQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withLocale != nil {
			_spec.Node.AddColumnOnce(user.FieldLocaleID)
		}
	}
