...
func main() {
	...
	opts := []schema.MigrateOption{
		...
		// per ADR-005, rollback is redeploying the previous image, never a
		// schema rollback, so there is no scenario where a dropped column
		// needs to still exist for an old binary to read.
